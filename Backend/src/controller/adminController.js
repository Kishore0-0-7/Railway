...
const createAdmin = async (req, res) => {
  ...
  const sanitizedMobile = mobile_number ? mobile_number.trim() : null;
...
const adminLogin = async (req, res) => {
  ...
  try {
    const query = `
      SELECT * FROM admin_accounts WHERE LOWER(email) = $1;
    `;
    const { rows } = await client.query(query, [normalizedEmail]);

    if (rows.length === 0) {
      console.warn("[adminLogin] Admin not found", { email: normalizedEmail });
...
const getBookingStats = async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(statsQuery, admin_id ? [admin_id] : []);
    ...
    res.status(200).json({
      ...
      stats: {
        ...
        avg_booking_hours:
          Math.round(parseFloat(rows[0].avg_booking_hours)) || 0,
